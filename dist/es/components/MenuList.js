...
const MenuList = ({
      ...
      if (!scrollNodes.menu) {
        scrollNodes.menu =
          (boundingBoxRef
            ...
            : getScrollAncestor(rootMenuRef.current)) || window;
      }
  ...
  useEffect(() => {
    ...
    if (!scrollNodes.anchors) {
      scrollNodes.anchors = [];
      let anchorScroll = getScrollAncestor(
        rootAnchorRef && rootAnchorRef.current,
      );
    ...
    const scrollObservers = scrollNodes.anchors.concat(
      viewScroll !== "initial" ? menuScroll : [],
    );
    scrollObservers.forEach((o) => o.addEventListener("scroll", handleScroll));
    return () =>
      scrollObservers.forEach((o) =>
  ...
  useEffect(() => {
    ...
    if (anchor) {
      resizeObserver.observe(anchor, resizeObserverOptions);
    }
  ...
  const menu = /*#__PURE__*/ jsxs("ul", {
    ...
    ...mergeProps(
      {
        onPointerEnter: parentSubmenuCtx && parentSubmenuCtx.off,
        onPointerMove,
        onPointerLeave,
        onKeyDown,
        onAnimationEnd,
